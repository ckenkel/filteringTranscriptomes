...
foreach $c (@cons)
	...
	system("makeblastdb -dbtype nucl -in tq.fasta");
	system ("legacy_blast.pl blastall -i $condb -d tq.fasta -p $myprog -a 16 -e 1 -F F -v $maxhits -b $maxhits -o tq.br --path /home/rcf-proj2/cdk1/local/ncbi-blast-2.2.22+/bin");
